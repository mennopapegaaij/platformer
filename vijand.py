...
class Spikes(Vijand):
    ...
    # Hoe groot één spike-punt is (2,5 keer zo groot als vroeger: was 16 breed, 18 hoog)
    PUNT_BREEDTE = 40
    PUNT_HOOGTE = 45
...
class Achtervolger(Vijand):
    ...
    def __init__(self, x, y):
        ...
        self.levens = 999999
        self._wiebel = 0
        self.stop_x = None               # x waar de boss stopt/doodgaat (None = nooit)
        self._gestopt = False            # is hij bij de stop-lijn aangekomen?
    ...
    def bijwerken(self, speler_x=None):
        ...
        if speler_x < self.x:
            self.x = self.start_x
            self._gestopt = False        # de boss leeft weer bij een herstart
            return
        if self._gestopt:
            return                       # bij de stop-lijn aangekomen -> hij blijft daar dood
        # Bij de ingestelde stop-plek gaat de boss dood (hij komt niet verder)
        if self.stop_x is not None and self.x + self.breedte / 2 >= self.stop_x:
            self.x = self.stop_x - self.breedte / 2
            self._gestopt = True
            return
    ...
    def raakt_speler(self, px, py, pw, ph):
        if self._gestopt:
            return False                 # gestopte (dode) boss doet geen kwaad meer
    ...
    def teken(self):
        ...
        arcade.draw_triangle_filled(x + 6, y + h, x + 18, y + h, x + 12, y + h + 14, rand)
        arcade.draw_triangle_filled(x + w - 6, y + h, x + w - 18, y + h, x + w - 12, y + h + 14, rand)
        oy = y + h * 0.72
        if self._gestopt:
            # X-ogen (verslagen)
            for dx in (-13, 13):
                arcade.draw_line(cx + dx - 6, oy - 6, cx + dx + 6, oy + 6, (40, 40, 50), 3)
                arcade.draw_line(cx + dx - 6, oy + 6, cx + dx + 6, oy - 6, (40, 40, 50), 3)
        else:
            # Boze rode ogen (wiebelen een beetje)
            import math as _m
            oy = oy + _m.sin(self._wiebel) * 2
            for dx in (-13, 13):
...
class BossLijn:
    """De stop-lijn waar de boss doodgaat. Alleen ter versiering (rood-witte lijn)."""

    is_spike = True                      # geen monster (telt niet mee, niet te schieten)
    levens = 999999

    def __init__(self, x):
        self.x = x                       # midden van de lijn
        self.breedte = 8
        self.hoogte = 2000
        self.y = 0
    ...
    def raakt_speler(self, px, py, pw, ph):
        return False                     # de lijn zelf doet niks

    def speler_springt_erop(self, px, py, pw, ph):
        return False

    def teken(self):
        # Een rood-witte streepjeslijn omhoog (zo zie je waar je veilig bent)
        for yy in range(0, 1600, 24):
            kleur = (230, 40, 40) if (yy // 24) % 2 == 0 else (255, 255, 255)
            arcade.draw_lrbt_rectangle_filled(self.x - 3, self.x + 3, yy, yy + 14, kleur)
