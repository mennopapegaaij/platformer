...
PORTAAL_SOORTEN = ["vlucht", "blok", "ufo", "bal", "golf", "robot", "spin", "heli",
                   ...
                   "voorspeller", "pendel", "blinde", "dubbelflip", "vijfkamp", "tienkamp", "vijftienkamp", "twintigkamp", "element", "elementkoning", "bouwmeester", "portaalschieter", "drakentemmer", "mierenkolonie", "evolutie", "schilder", "chemicus", "bommenlegger", "boogschutter", "spinnenheld", "tijdreiziger", "robotbouwer", "dierentemmer", "fabriek",
                   "eigen", "dubbel", "enkel"]
PORTAAL_NAAM = {"vlucht": "Vlieg", "blok": "Blok", "ufo": "UFO", "bal": "Bal",
                ...
                "spook": "Spook", "vleermuis": "Vleermuis", "zombie": "Zombie",
                "pompoenkop": "Pompoen", "voorspeller": "Voorspel", "pendel": "Pendel",
                "blinde": "Blind", "dubbelflip": "DubbFlip", "vijfkamp": "Vijfkamp", "tienkamp": "Tienkamp", "vijftienkamp": "15kamp", "twintigkamp": "20kamp", "element": "Element", "elementkoning": "Koning", "bouwmeester": "Bouwer", "portaalschieter": "PortSchiet", "drakentemmer": "Draak", "mierenkolonie": "Mieren", "evolutie": "DNA", "schilder": "Schilder", "chemicus": "Chemicus", "bommenlegger": "Bommen", "boogschutter": "Boog", "spinnenheld": "Spin", "tijdreiziger": "Tijd", "robotbouwer": "RoboBouw", "dierentemmer": "Temmer", "fabriek": "Fabriek", "eigen": "Mijn",
...
def teken_item(soort, x, y, grootte, rotatie=0):
    ...
    if soort == "grond":
        ...
        cx = x + g / 2
        arcade.draw_lrbt_rectangle_filled(cx - 8, cx + 8, y + 6, y + g * 0.55, (120, 200, 255))
        ...
        cx = x + g / 2
        arcade.draw_circle_filled(cx - 6, y + g - 14, 3.5, (255, 255, 255))
        arcade.draw_circle_filled(cx + 6, y + g - 14, 3.5, (255, 255, 255))
        arcade.draw_circle_filled(cx - 6, y + g - 14, 2, (220, 30, 30))
        arcade.draw_circle_filled(cx + 6, y + g - 14, 2, (220, 30, 30))
        arcade.draw_line(x + 8, y + 12, x + g - 8, y + 12, (255, 255, 255), 2)
    elif soort == "draad":
...
class BouwerView(arcade.View):
    ...
    def _verf_alles(self):
        ...
        if s.startswith("b_"):
            # Beweeg-kwast: alles wel/niet in deze stijl laten bewegen
            stijl = s[2:]
            heeft_alles = all(self.beweeg.get(c) == stijl for c in alle)
            if heeft_alles:
                for c in alle:
                    if self.beweeg.get(c) == stijl:
                        self.beweeg.pop(c, None)
                self._melding = "Beweging overal uit"
            else:
                for c in alle:
                    self.beweeg[c] = stijl
            ...
            if heeft_alles:                              # andersom: overal weer normaal
                for c in alle:
                    if self.verf.get(c) == s:
                        self.verf.pop(c, None)
                self._melding = "Alles weer normaal"
            else:
                for c in alle:
                    self.verf[c] = s
                self._melding = ("👻 Alles onzichtbaar" if s == "onzichtbaar"
                                 else "🚶 Overal doorheen lopen")
        else:
    ...
    def _teken_items(self):
        ...
        for (kol, rij), verf_soort in self.verf.items():
            ...
            elif verf_soort == "doorheen":
                # lichtgroen waas met een pijl -> hier loop je dwars doorheen
                arcade.draw_lrbt_rectangle_filled(sx, sx + CEL, sy, sy + CEL, (150, 255, 170, 80))
                arcade.draw_lrbt_rectangle_outline(sx, sx + CEL, sy, sy + CEL, (80, 220, 120, 200), 2)
                cy = sy + CEL / 2
    ...
    def _bouw_level(self):
        ...
        for (kol, rij), soort in self.grid.items():
            ...
            if soort == "grond":
                ...
                s = soort.split("_", 1)[1] if "_" in soort else "gewoon"
                if s == "schuinop":
                    plat = SchuinBlok(wx, wy, CEL, CEL, "op")
                elif s == "schuinaf":
                    plat = SchuinBlok(wx, wy, CEL, CEL, "af")
                elif s == "half":
                    plat = BlokPlatform(wx, wy, CEL, CEL // 2)
                elif s == "stuiter":
